...
# to subset into friend and nonfriend 
...
table<- finalDat %>% count(id, typeHelp, chooseFriend, sort=TRUE)
levels(table$chooseFriend)<-c(NA, "f", "n","n")
levels(table$typeHelp)<- c(NA, "comfort", "comfort", "goal","goal","none","none", "share")
View(table)  


KatePSBdata <- read.csv("~/Desktop/KatePSBdata.csv")
colnames(KatePSBdata)
summary(aov(n ~ chooseFriend *typeHelp+ Error(id/chooseFriend), data= KatePSBdata))

demographics<- cbind.data.frame(ps_dat$databraryID,ps_dat$study, ps_dat$age)

colnames(demographics)<- c("id", "study", "age")
psb<-left_join(demographics,KatePSBdata)

summary(aov(n ~ chooseFriend *typeHelp+age+study+ Error(id), data= psb))
```
#Question 3 
```{r}
#Data manipulation 
